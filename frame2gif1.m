function frame2gif1(volume,videoname,delaytime,resizeRate,height)
...
%     [X,map]=rgb2ind(im,256);
    if i==1
        imwrite(im2uint8(imresize(im,1)),videoname,'DelayTime',delaytime,'LoopCount',Inf);
    else
        imwrite(im2uint8(imresize(im,1)),videoname,'WriteMode','append','DelayTime',delaytime);
    end
end